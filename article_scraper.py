...
class ArticlesScraper:
    ...
    def _get_all_article_links(self):
        ...
        for sitemap_url in tqdm(article_sitemap_urls, desc="Parsing Article Sitemaps"):
            self.rate_limiter.wait()
            try:
    ...
    def _scrape_article_text(self, article_url: str):
        try:
            self.rate_limiter.wait()
            ...
            for p_tag in content_div.find_all('p'):
                ...
                if self._is_valid_paragraph(text):
                    cleaned_text = utils.preprocess_text(text=text, mask_name=False)
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(os.path.dirname(__file__), "data", "scraped_articles.txt")
    ...
    # 20838 total articles from sitemap (before filtering) (matches 21 pages containing max 1000 articles)